...
test_that("Returns an error", {
  ...
  expect_that(knapsack_brute_force(x = k1, W = 3500),
              throws_error("x dont have two variables w and v"))
  
  k2 <- data.frame(w=c(1,2,3),w2=c(4,5,6))
  expect_that(knapsack_brute_force(x = k2, W = 2000),
              throws_error("x dont have two variables w and v"))
 
  expect_that(knapsack_brute_force(x = knapsack_objects[1:8,], W = -5),
              throws_error("Not numeric and/or positive W"))
  
  expect_that(knapsack_brute_force(x = knapsack_objects[1:8,], W = "W"),
              throws_error("Not numeric and/or positive W"))
  
  k3 <- data.frame(w=c(1,-2,3),v=c(4,5,6))
  expect_that(knapsack_brute_force(x = k3, W = 3500),
              throws_error("Not only positive values in data.frame"))
  
  k4 <- data.frame(w=c(1,2,3),v=c(4,-5,6))
  expect_that(knapsack_brute_force(x = k4, W = 3500),
              throws_error("Not only positive values in data.frame"))
})
